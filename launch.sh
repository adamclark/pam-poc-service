...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "pam-poc-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name pam-poc-service apps/pam-poc-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app pam-poc-service:1.0-SNAPSHOT
    oc expose svc/pam-poc-service
else

	echo "Launching the application locally..."
	pattern="pam-poc-service"
	files=( $pattern )
	cd ${files[0]}
