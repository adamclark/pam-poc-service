...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "pam-poc-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name pam-poc-service apps/pam-poc-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app pam-poc-service:1.0-SNAPSHOT
		call oc expose svc/pam-poc-service
	) else (
